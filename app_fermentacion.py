...
with st.sidebar:
    ...
    mostrar_temp_camisa = st.checkbox("Mostrar temperatura de camisa (T2)", value=False)
...
for uploaded_file in uploaded_files:
    try:
        ...
        df_temp_camisa = df[df['VarName'].str.contains('T2.Output_registro|T2_Output_registro', na=False)]

        df_temp['Valor'] = pd.to_numeric(df_temp['VarValue'], errors='coerce')
        df_pres['Valor'] = pd.to_numeric(df_pres['VarValue'], errors='coerce')
        df_temp_camisa['Valor'] = pd.to_numeric(df_temp_camisa['VarValue'], errors='coerce')
...
for idx, nombre in enumerate(lotes_seleccionados):
    ...
    df_temp_camisa = data['temp_camisa'].sort_values('Tiempo').reset_index(drop=True) if len(data['temp_camisa']) else pd.DataFrame()
    ...
    if tiempo_relativo:
        ...
        x_temp_camisa = (df_temp_camisa['Tiempo'] - inicio).dt.total_seconds() / 3600 if len(df_temp_camisa) else []
        xlabel = "Horas"
    else:
        x_temp = df_temp['Tiempo']
        x_pres = df_pres['Tiempo']
        x_temp_camisa = df_temp_camisa['Tiempo']
        xlabel = "Fecha/Hora"
...
fig_temp.update_layout(
    title="Temperatura (T1) y Temperatura de Camisa (T2)",
    xaxis_title=xlabel,
    yaxis_title="°C",
    hovermode="x unified",
    height=600
...
if mostrar_analisis and mostrar_temp_camisa:
